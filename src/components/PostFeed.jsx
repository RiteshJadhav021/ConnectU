...
const PostFeed = () => {
  ...
  const handleLike = async (postId) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/tpo/posts/${postId}/like`, {
        ...
        body: JSON.stringify({ userId: user.email || user._id || user.id })
      });
      if (res.ok) {
        const data = await res.json();
  ...
  if (loading) return <div className="text-center py-8 text-indigo-600 font-semibold">Loading posts...</div>;
  if (!posts.length) return <div className="text-center py-8 text-gray-500">No posts found.</div>;
  ...
  return (
    <div className="max-w-2xl mx-auto mt-8 relative">
        ...
        {posts.map((post, idx) => (
          <div key={post._id || idx} className="bg-white rounded-2xl shadow p-6 border border-gray-100">
            ...
            <div className="flex gap-4 mt-2">
              ...
              <button 
                className="flex items-center gap-1 text-gray-600 hover:text-blue-500 px-3 py-1 rounded-full border border-gray-200"
      ...
      {/* Comment Modal */}
      {activeCommentPost && (
        <div className="fixed top-0 right-0 h-full w-full md:w-96 bg-white shadow-2xl border-l border-blue-200 z-50 flex flex-col">
          <button 
            className="self-end m-4 text-2xl text-gray-400 hover:text-red-500 font-bold" 
          ...
          <div className="p-6 flex-1 flex flex-col">
            ...
            <ul className="pl-2 mt-2 space-y-3 overflow-y-auto max-h-72">
