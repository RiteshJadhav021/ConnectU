...
const TPOMyPosts = () => {
    ...
    const fetchPosts = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // Only show posts by current TPO
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-cyan-100 via-white to-blue-200 items-center">
      <div className="w-full max-w-xl mt-12 p-6 bg-white rounded-2xl shadow-xl border border-blue-200">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-blue-700">My Posts</h2>
          <button className="text-gray-400 hover:text-blue-600 text-xl font-bold" onClick={() => navigate(-1)} aria-label="Back">×</button>
        </div>
        {loading ? (
            ...
            {posts.map(post => (
              <li key={post._id} className="py-4 flex flex-col gap-2">
                <div className="flex items-center gap-2">
                  <span className="font-semibold text-blue-700">{post.content}</span>
                  <button
